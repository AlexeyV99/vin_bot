...
from func.f_vininfo import  vin_info_search
from func.f_my_vin import my_vin_search
from func.brands.kia import info_kia


def vin_handler(message) -> None:
    logger.info(f'Пользователь {message.from_user.first_name} (id:{message.from_user.id}) выполнил команду <vin>')
    bot.register_next_step_handler(message, vin_info)
    bot.send_message(message.chat.id, 'Введите ВИН:')
...
def vin_info(message):
    vin = vin_check(message.text)
    if vin:
        logger.info(f'Пользователь {message.from_user.first_name} (id:{message.from_user.id}) проверяет вин={vin}')
        bot.send_message(message.chat.id, f'<b>== VIN: {vin} ==</b>', parse_mode='html')
        bot.send_message(message.chat.id, f'{vin_info_search(message, vin)}', parse_mode='html')
        info = my_vin_search(message, vin)
        bot.send_message(message.chat.id, f'{info}', parse_mode='html')
        if 'KIA' in info.upper():
            bot.send_message(message.chat.id, f'{info_kia(message, vin)}', parse_mode='html')
    else:
        bot.send_message(message.chat.id, 'Ошибка ввода ВИН!')
...
def register_vin_handlers(bot) -> None:
    bot.register_message_handler(vin_handler, commands=['vin'])
